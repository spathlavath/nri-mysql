...
func processExecutionMetricsIngestion(e *integration.Entity, args arguments.ArgumentList, metricObject map[string]interface{}) {
	...
	metricsMap := map[string]struct {
		...
		"query_id":   {common_utils.GetStringValueSafe(metricObject["query_id"].(string)), metric.ATTRIBUTE},
		"query_text": {common_utils.GetStringValueSafe(metricObject["query_text"].(string)), metric.ATTRIBUTE},
		// "total_cost": {common_utils.GetFloat64ValueSafe(metricObject["total_cost"].(string)), metric.GAUGE},
		// "step_id":        {common_utils.GetInt64ValueSafe(metricObject["step_id"].(string)), metric.GAUGE},
